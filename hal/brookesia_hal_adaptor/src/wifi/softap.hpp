...
public:
    ...
    bool on_hal_scan_ap_infos_updated(const std::vector<wifi::ScanApInfo> &ap_infos);
    ...
    static uint8_t get_ap_best_channel(const std::vector<wifi::ScanApInfo> &ap_infos);

private:
    bool do_soft_ap_start(const wifi::SoftApParams *params = nullptr);
    void do_softap_stop();

    bool start_soft_ap_setup_task();
    void stop_soft_ap_setup_task();
    bool start_soft_ap_event_task(wifi::SoftApEvent event);
    void stop_soft_ap_event_task();
    bool is_softap_setup_task_running()
    {
        return (softap_setup_task_ != 0);
    }
    bool is_softap_setup_ready();
    void set_softap_target_event(wifi::SoftApEvent event);
    wifi::SoftApEvent get_softap_target_event()
    {
    ...
    std::vector<wifi::ScanApInfo> provision_scan_ap_infos_;
