...
- feat(logging): print the component version during device initialization.
...
- feat(api): add per-device and global pre-init / post-deinit callbacks
- feat(api): add auto-registration macros with deferred attachment to tolerate static init order
- feat(test): add `test_callbacks.cpp` covering the new callback APIs and macros
