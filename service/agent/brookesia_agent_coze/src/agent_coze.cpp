...
std::expected<std::string, std::string> make_storage_namespace(std::string_view raw_namespace)
{
    ...
    if (!result) {
        return std::unexpected(
                   (boost::format("Failed to make Coze Storage namespace '%1%': %2%") %
...
std::expected<std::string, std::string> make_storage_key(std::string_view raw_key)
{
    ...
    if (!result) {
        return std::unexpected(
                   (boost::format("Failed to make Coze Storage key '%1%': %2%") %
...
bool Coze::on_interrupt_speaking()
{
    ...
    BROOKESIA_LOGI(
        "Interrupt Coze speaking: chat_started(%1%), chat_connected(%2%)",
        is_chat_started(), is_chat_connected()
    );
    ...
    BROOKESIA_LOGI("Interrupted Coze speaking");
