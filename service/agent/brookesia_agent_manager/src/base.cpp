...
bool Base::feed_audio_decoder_data(const uint8_t *data, size_t data_size)
{
    ...
    const bool completed = wait_context.cv.wait_for(
                               lock, std::chrono::milliseconds(AGENT_AUDIO_WRITE_COMPLETION_TIMEOUT_MS),
    [&wait_context]() {
...
bool Base::start_audio_encoder()
{
    ...
    auto afe_event_happened_slot = [this](df::AudioAfeEvent afe_event) {
        ...
        case df::AudioAfeEvent::WakeStart: {
            ...
            BROOKESIA_LOGI(
                "AFE WakeStart: suspended(%1%), listening(%2%), speaking(%3%), supports_interrupt(%4%), "
                "chat_mode(%5%)",
                is_suspended(), is_listening(), is_speaking(), supports_interrupt,
                BROOKESIA_DESCRIBE_TO_STR(get_chat_mode())
            );
            if (is_suspended()) {
                BROOKESIA_LOGD("Suspended, skip");
                return;
            }
            if (is_speaking() && supports_interrupt) {
                if (!do_interrupt_speaking()) {
                    BROOKESIA_LOGW("Failed to interrupt speaking on AFE WakeStart");
                }
            } else if (is_speaking()) {
                BROOKESIA_LOGW("Speaking on AFE WakeStart, but InterruptSpeaking is unsupported");
        ...
        case df::AudioAfeEvent::WakeEnd:
            ...
            if (is_speaking()) {
                BROOKESIA_LOGD("Speaking, skip WakeEnd");
                return;
            }
