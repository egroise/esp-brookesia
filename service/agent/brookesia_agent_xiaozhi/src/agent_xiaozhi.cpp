...
using XiaoZhiHelper = service::helper::XiaoZhi;
...
std::expected<std::string, std::string> make_keystore_namespace(std::string_view raw_namespace)
{
    auto result = StorageHelper::make_kv_namespace({raw_namespace});
    if (!result) {
        return std::unexpected(
                   (boost::format("Failed to make XiaoZhi keystore namespace '%1%': %2%") %
...
std::expected<std::string, std::string> make_keystore_key(std::string_view raw_key)
{
    auto result = StorageHelper::make_kv_key({raw_key});
    if (!result) {
        return std::unexpected(
                   (boost::format("Failed to make XiaoZhi keystore key '%1%': %2%") %
...
esp_err_t on_keystore_nvs_open(const char *name_space, bool read_write, nvs_ops_handle_t *out_handle)
{
    ...
    auto namespace_result = make_keystore_namespace(name_space);
    BROOKESIA_CHECK_FALSE_RETURN(namespace_result, ESP_FAIL, "%1%", namespace_result.error());

    *out_handle = get_keystore_nvs_handle_by_nspace(std::move(namespace_result.value()));
...
esp_err_t on_keystore_nvs_get_str(nvs_ops_handle_t handle, const char *key, char *out_value, size_t *length)
{
    ...
    auto key_result = make_keystore_key(key);
    BROOKESIA_CHECK_FALSE_RETURN(key_result, ESP_FAIL, "%1%", key_result.error());
    auto result = StorageHelper::get_key_value<std::string>(nspace, key_result.value());
...
esp_err_t on_keystore_nvs_set_str(nvs_ops_handle_t handle, const char *key, const char *value)
{
    ...
    auto key_result = make_keystore_key(key);
    BROOKESIA_CHECK_FALSE_RETURN(key_result, ESP_FAIL, "%1%", key_result.error());
    auto result = StorageHelper::save_key_value(nspace, key_result.value(), value);
    BROOKESIA_CHECK_FALSE_RETURN(
...
esp_err_t on_keystore_nvs_get_i32(nvs_ops_handle_t handle, const char *key, int32_t *out_value)
{
    ...
    auto key_result = make_keystore_key(key);
    BROOKESIA_CHECK_FALSE_RETURN(key_result, ESP_FAIL, "%1%", key_result.error());
    auto result = StorageHelper::get_key_value<int32_t>(nspace, key_result.value());
    if (!result && result.error().find("ESP_ERR_NVS_NOT_FOUND")) {
        BROOKESIA_LOGW("Key '%1%' not found in NVS, returning empty string", key);
        return ESP_ERR_NOT_FOUND;
    }
...
esp_err_t on_keystore_nvs_erase_key(nvs_ops_handle_t handle, const char *key)
{
    ...
    auto key_result = make_keystore_key(key);
    BROOKESIA_CHECK_FALSE_RETURN(key_result, ESP_FAIL, "%1%", key_result.error());
    auto result = StorageHelper::erase_keys(nspace, {key_result.value()});
    BROOKESIA_CHECK_FALSE_RETURN(
...
esp_err_t on_keystore_nvs_erase_all(nvs_ops_handle_t handle)
{
    ...
    auto result = StorageHelper::erase_keys(nspace, {});
    BROOKESIA_CHECK_FALSE_RETURN(
...
bool XiaoZhi::on_interrupt_speaking()
{
    ...
    BROOKESIA_LOGI(
        "Interrupt XiaoZhi speaking: audio_channel_opened(%1%)",
        is_xiaozhi_audio_channel_opened()
    );
    ...
    BROOKESIA_LOGI("Interrupted XiaoZhi speaking");
