...
/**
 * @brief Get the schema value category for one runtime function value.
 *
 * @param value Runtime function value.
 * @return FunctionValueType Matching top-level schema type.
 */
inline FunctionValueType get_function_value_type(const FunctionValue &value)
{
    if (std::holds_alternative<bool>(value)) {
        return FunctionValueType::Boolean;
    }
    if (std::holds_alternative<double>(value)) {
        return FunctionValueType::Number;
    }
    if (std::holds_alternative<std::string>(value)) {
        return FunctionValueType::String;
    }
    if (std::holds_alternative<boost::json::object>(value)) {
        return FunctionValueType::Object;
    }
    if (std::holds_alternative<boost::json::array>(value)) {
        return FunctionValueType::Array;
    }
    return FunctionValueType::RawBuffer;
}
...
struct FunctionParameterSchema {
    ...
    {
        return get_function_value_type(value) == type;
    }
    /**
...
/**
 * @brief Schema description for one function's successful return value.
 */
struct FunctionReturnSchema {
    FunctionValueType type = FunctionValueType::String;
    std::string description = "";
};
BROOKESIA_DESCRIBE_STRUCT(FunctionReturnSchema, (), (type, description));
...
struct FunctionSchema {
    ...
    std::optional<FunctionReturnSchema> return_value = std::nullopt;
};
BROOKESIA_DESCRIBE_STRUCT(
    FunctionSchema, (), (name, description, parameters, require_scheduler, default_timeout_ms, return_value)
);
