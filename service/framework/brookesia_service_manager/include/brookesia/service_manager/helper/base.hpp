...
public:
    ...
    static const EventSchema *get_event_schema(EventIdType event_id)
    {
        ...
        const auto event_index = static_cast<std::size_t>(BROOKESIA_DESCRIBE_ENUM_TO_NUM(event_id));
        if (event_index >= event_schemas.size()) {
 ...
 *
 * Example:
 * BROOKESIA_SERVICE_HELPER_FUNC_HANDLER_1(MyService, MyService::FunctionId::Play, std::string, function_play(PARAM))
 */
