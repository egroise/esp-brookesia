...
public:
    ...
    std::vector<FunctionSchema> get_function_schemas() override
    {
        return {
            ...
            FunctionSchema{
                .name = "delayed_echo",
                .description = "Echo a string after a short delay",
                .parameters = {
                    FunctionParameterSchema{.name = "message", .description = "Message to echo", .type = FunctionValueType::String},
                },
            ...
            FunctionSchema{
                ...
                .description = "Return data without declaring a return value",
                .parameters = {},
            },
            FunctionSchema{
                .name = "wrong_return_type",
                .description = "Return a mismatched value type",
                .parameters = {},
                .return_value = FunctionReturnSchema{
                    .type = FunctionValueType::Number,
    ...
    FunctionHandlerMap get_function_handlers() override
    {
        return {
            ...
            },
            {
                "delayed_echo",
                [this](const FunctionParameterMap & args) -> FunctionResult {
                    return handle_delayed_echo(args);
                }
            },
            {
                "unexpected_return",
                [](const FunctionParameterMap &) -> FunctionResult {
                    return FunctionResult{
                        .success = true,
                        .data = FunctionValue(std::string("unexpected")),
                    };
                }
            },
            {
                ...
                [](const FunctionParameterMap &) -> FunctionResult {
                    return FunctionResult{
                        .success = true,
                        .data = FunctionValue(std::string("wrong")),
                    };
                }
            },
        };
...
private:
    ...
    FunctionResult handle_delayed_echo(const FunctionParameterMap &args)
    {
        esp_brookesia::lib_utils::test_adapter::delay_ms(80);
        return handle_echo(args);
    }
...
BROOKESIA_TEST_CASE(test_apis_bind_service, "Test APIs: bind service", "[brookesia][service][api][bind]")
{
...
BROOKESIA_TEST_CASE(test_apis_bind_non_existent_service, "Test APIs: bind non-existent service", "[brookesia][service][api][bind_not_exist]")
{
...
BROOKESIA_TEST_CASE(test_apis_binding_auto_release, "Test APIs: binding auto release", "[brookesia][service][api][bind_auto_release]")
{
...
BROOKESIA_TEST_CASE(test_apis_multiple_bindings_share_same_instance, "Test APIs: multiple bindings share same instance", "[brookesia][service][api][multiple_bindings]")
{
...
BROOKESIA_TEST_CASE(test_apis_binding_move_semantics, "Test APIs: binding move semantics", "[brookesia][service][api][bind_move]")
{
...
BROOKESIA_TEST_CASE(test_apis_service_lifecycle_callbacks, "Test APIs: service lifecycle callbacks", "[brookesia][service][api][lifecycle]")
{
...
BROOKESIA_TEST_CASE(test_apis_service_state_after_bind, "Test APIs: service state after bind", "[brookesia][service][api][state_after_bind]")
{
...
BROOKESIA_TEST_CASE(test_apis_shared_service_lifecycle_with_ref_counting, "Test APIs: shared service lifecycle with ref counting", "[brookesia][service][api][shared_lifecycle]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_service_function_add, "Test APIs: call service function - add", "[brookesia][service][api][call_function_add]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_service_function_echo, "Test APIs: call service function - echo", "[brookesia][service][api][call_function_echo]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_uses_schema_default_timeout, "Test APIs: call function uses schema default timeout", "[brookesia][service][api][call_function_timeout]")
{
    BROOKESIA_LOGI("=== Test call function uses schema default timeout ===");
    ...
    FunctionParameterMap args;
    args["message"] = FunctionValue("slow");

    auto timeout_result = service->call_function_sync("delayed_echo", std::move(args));
    TEST_ASSERT_FALSE(timeout_result.success);
    TEST_ASSERT_TRUE(timeout_result.error_message.find("Wait timeout after 20 ms") != std::string::npos);

    FunctionParameterMap explicit_args;
    explicit_args["message"] = FunctionValue("slow-ok");
    auto result = service->call_function_sync("delayed_echo", std::move(explicit_args), 300);
    TEST_ASSERT_TRUE_MESSAGE(result.success, result.error_message.c_str());
    TEST_ASSERT_TRUE(result.has_data());
    TEST_ASSERT_EQUAL_STRING("slow-ok", std::get<std::string>(*result.data).c_str());

    service_manager.stop();
    service_manager.deinit();
}

BROOKESIA_TEST_CASE(test_apis_call_function_validates_return_schema, "Test APIs: call function validates return schema", "[brookesia][service][api][call_function_return_schema]")
{
    BROOKESIA_LOGI("=== Test call function validates return schema ===");
    ...
    auto unexpected_result = service->call_function_sync("unexpected_return", FunctionParameterMap{}, 100);
    TEST_ASSERT_FALSE(unexpected_result.success);
    TEST_ASSERT_TRUE(unexpected_result.error_message.find("returned data") != std::string::npos);

    auto wrong_type_result = service->call_function_sync("wrong_return_type", FunctionParameterMap{}, 100);
    TEST_ASSERT_FALSE(wrong_type_result.success);
    TEST_ASSERT_TRUE(wrong_type_result.error_message.find("invalid data type") != std::string::npos);

    service_manager.stop();
    service_manager.deinit();
}

BROOKESIA_TEST_CASE(test_apis_call_non_existent_function, "Test APIs: call non-existent function", "[brookesia][service][api][call_function_not_exist]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_with_json_object_add, "Test APIs: call function with json object - add", "[brookesia][service][api][call_function_json_add]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_with_json_object_echo, "Test APIs: call function with json object - echo", "[brookesia][service][api][call_function_json_echo]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_with_vector_add, "Test APIs: call function with vector - add", "[brookesia][service][api][call_function_vector_add]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_with_vector_echo, "Test APIs: call function with vector - echo", "[brookesia][service][api][call_function_vector_echo]")
{
...
BROOKESIA_TEST_CASE(test_apis_call_function_with_vector_wrong_argument_count, "Test APIs: call function with vector - wrong argument count", "[brookesia][service][api][call_function_vector_wrong_count]")
{
...
BROOKESIA_TEST_CASE(test_apis_subscribe_and_publish_event, "Test APIs: subscribe and publish event", "[brookesia][service][api][event]")
{
    ...
    esp_brookesia::lib_utils::test_adapter::delay_ms(100);
...
BROOKESIA_TEST_CASE(test_apis_publish_event_with_string_data, "Test APIs: publish event with string data", "[brookesia][service][api][event_string]")
{
...
BROOKESIA_TEST_CASE(test_apis_multiple_event_subscribers, "Test APIs: multiple event subscribers", "[brookesia][service][api][event_multiple_subscribers]")
{
...
BROOKESIA_TEST_CASE(test_apis_publish_event_with_json_object, "Test APIs: publish event with json object", "[brookesia][service][api][event_json_object]")
{
...
BROOKESIA_TEST_CASE(test_apis_publish_event_with_data_map, "Test APIs: publish event with data map", "[brookesia][service][api][event_event_items]")
{
...
BROOKESIA_TEST_CASE(test_apis_publish_event_with_json_object_multiple_fields, "Test APIs: publish event with json object - multiple fields", "[brookesia][service][api][event_json_multiple]")
{
...
BROOKESIA_TEST_CASE(test_apis_is_initialized, "Test APIs: is_initialized", "[brookesia][service][api][is_initialized]")
{
...
BROOKESIA_TEST_CASE(test_apis_is_running, "Test APIs: is_running", "[brookesia][service][api][is_running]")
{
...
BROOKESIA_TEST_CASE(test_apis_bind_before_init, "Test APIs: bind before init", "[brookesia][service][api][bind_before_init]")
{
...
BROOKESIA_TEST_CASE(test_apis_bind_with_empty_name, "Test APIs: bind with empty name", "[brookesia][service][api][bind_empty_name]")
{
...
BROOKESIA_TEST_CASE(test_apis_concurrent_bind_services, "Test APIs: concurrent bind services", "[brookesia][service][api][concurrent_bind]")
{
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        threads.create_thread([&bind_success_count]() {
            ...
            esp_brookesia::lib_utils::test_adapter::delay_ms(10);
        });
    }
...
BROOKESIA_TEST_CASE(test_apis_async_call_function_add, "Test APIs: async call function - add", "[brookesia][service][api][call_function_async_add]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_function_echo, "Test APIs: async call function - echo", "[brookesia][service][api][call_function_async_echo]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_function_with_vector, "Test APIs: async call function with vector", "[brookesia][service][api][call_function_async_vector]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_function_with_json_object, "Test APIs: async call function with json object", "[brookesia][service][api][call_function_async_json]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_multiple_functions_concurrently, "Test APIs: async call multiple functions concurrently", "[brookesia][service][api][call_function_async_concurrent]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_with_custom_timeout_check, "Test APIs: async call with custom timeout check", "[brookesia][service][api][call_function_async_timeout]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_vs_sync_call_comparison, "Test APIs: async vs sync call comparison", "[brookesia][service][api][async_vs_sync]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_non_existent_function, "Test APIs: async call non-existent function", "[brookesia][service][api][call_function_async_not_exist]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_with_mixed_sync_and_async, "Test APIs: async call with mixed sync and async", "[brookesia][service][api][mixed_sync_async]")
{
...
BROOKESIA_TEST_CASE(test_apis_async_call_before_service_running, "Test APIs: async call before service running", "[brookesia][service][api][call_function_async_not_running]")
{
...
BROOKESIA_TEST_CASE(test_apis_stress_rapid_bind_release, "Test APIs: stress - rapid bind release", "[brookesia][service][api][stress_bind_release]")
{
...
BROOKESIA_TEST_CASE(test_apis_stress_rapid_async_calls, "Test APIs: stress - rapid async calls", "[brookesia][service][api][stress_async_calls]")
{
...
#endif
    constexpr auto result_timeout = std::chrono::seconds(10);
    ...
    int success_count = 0;
    for (int i = 0; i < iterations; i++) {
        const auto status = futures[i].wait_for(result_timeout);
        TEST_ASSERT_TRUE(status == std::future_status::ready);
        auto result = futures[i].get();
        if (result.success) {
            success_count++;
            TEST_ASSERT_EQUAL_DOUBLE(static_cast<double>(i + 1), std::get<double>(*result.data));
