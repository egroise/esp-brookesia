...
BROOKESIA_TEST_CASE(test_servicewifi_state_transitions, "Test ServiceWifi - state transitions", "[service][wifi][state]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_scan_functionality, "Test ServiceWifi - scan functionality", "[service][wifi][scan]")
{
    ...
    esp_brookesia::lib_utils::test_adapter::delay_ms(1000);
...
BROOKESIA_TEST_CASE(
    test_servicewifi_softap_params_round_trip,
    "Test ServiceWifi - SoftAP parameter handlers match schemas",
    "[service][wifi][softap][params]"
)
{
    BROOKESIA_TIME_PROFILER_SCOPE("test_service_wifi_softap_params");
    BROOKESIA_LOGI("=== Test ServiceWifi - SoftAP parameter handler mapping ===");
    ...
    const WifiHelpler::SoftApParams expected{
        .ssid = "Brookesia-Test",
        .password = "password",
        .max_connection = 3,
        .channel = 6,
    };
    auto set_result = WifiHelpler::call_function_sync<void>(
                          WifiHelpler::FunctionId::SetSoftApParams,
                          BROOKESIA_DESCRIBE_TO_JSON(expected).as_object()
                      );
    TEST_ASSERT_TRUE_MESSAGE(
        set_result.has_value(),
        set_result ? "SetSoftApParams succeeded" : set_result.error().c_str()
    );

    auto get_result = WifiHelpler::call_function_sync<boost::json::object>(
                          WifiHelpler::FunctionId::GetSoftApParams
                      );
    TEST_ASSERT_TRUE_MESSAGE(
        get_result.has_value(),
        get_result ? "GetSoftApParams succeeded" : get_result.error().c_str()
    );

    WifiHelpler::SoftApParams actual;
    TEST_ASSERT_TRUE_MESSAGE(
        BROOKESIA_DESCRIBE_FROM_JSON(*get_result, actual),
        "Failed to deserialize returned SoftAP parameters"
    );
    TEST_ASSERT_TRUE_MESSAGE(actual == expected, "Returned SoftAP parameters do not match the configured values");
}

BROOKESIA_TEST_CASE(test_servicewifi_set_connect_ap, "Test ServiceWifi - set connect AP", "[service][wifi][connect]")
{
    ...
    std::vector<service::LocalTestItem> test_items = {
        ...
        service::LocalTestItem{
            ...
            .method = BROOKESIA_DESCRIBE_TO_STR(WifiHelpler::FunctionId::SetConnectAp),
            .params = boost::json::object{
                {"SSID", "VeryLongSSIDNameThatExceedsNormalLength"},
...
BROOKESIA_TEST_CASE(test_servicewifi_connect_and_manual_disconnect_no_auto_reconnect, "Test ServiceWifi - connect and manual disconnect (no auto-reconnect)", "[service][wifi][connect][scenario1]")
{
    ...
    esp_brookesia::lib_utils::test_adapter::delay_ms(3000);
    {
        std::lock_guard<std::mutex> lock(collector.mutex);
...
BROOKESIA_TEST_CASE(test_servicewifi_stop_and_start_with_auto_reconnect, "Test ServiceWifi - stop and start with auto-reconnect", "[service][wifi][connect][scenario2]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_rapid_connect_and_disconnect, "Test ServiceWifi - rapid connect and disconnect", "[service][wifi][connect][disconnect][rapid]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_connect_to_non_existent_ssid_and_verify_auto_reconnect, "Test ServiceWifi - connect to non-existent SSID and verify auto-reconnect", "[service][wifi][connect][scenario4]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_switch_connection_from_test_wifi_ssid1_to_test_wifi_ssid2, "Test ServiceWifi - switch connection from TEST_WIFI_SSID1 to TEST_WIFI_SSID2", "[service][wifi][connect][scenario3]")
{
    ...
    std::vector<service::LocalTestItem> test_items2 = {
        service::LocalTestItem{
            ...
            .method = BROOKESIA_DESCRIBE_TO_STR(WifiHelpler::FunctionId::SetConnectAp),
            .params = boost::json::object{
                {"SSID", TEST_WIFI_SSID2},
                {"Password", TEST_WIFI_PASSWORD2}
            }
        },
        service::LocalTestItem{
...
BROOKESIA_TEST_CASE(test_servicewifi_repeatedly_switch_between_test_wifi_ssid1_and_test_wifi_ssid2, "Test ServiceWifi - repeatedly switch between TEST_WIFI_SSID1 and TEST_WIFI_SSID2", "[service][wifi][connect][scenario5]")
{
    ...
    // Give additional time for any delayed events
    esp_brookesia::lib_utils::test_adapter::delay_ms(2000);
...
BROOKESIA_TEST_CASE(test_servicewifi_connect_a_invalid_ap, "Test ServiceWifi - connect a invalid AP", "[service][wifi][connect][invalid_ap]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_error_handling_invalid_parameters, "Test ServiceWifi - error handling: invalid parameters", "[service][wifi][error][invalid_params]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_error_handling_invalid_state_transitions, "Test ServiceWifi - error handling: invalid state transitions", "[service][wifi][error][invalid_state]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_error_handling_rapid_state_changes, "Test ServiceWifi - error handling: rapid state changes", "[service][wifi][error][rapid_changes]")
{
    ...
    esp_brookesia::lib_utils::test_adapter::delay_ms(500);
}

// ==================== Stress Tests ====================

BROOKESIA_TEST_CASE(test_servicewifi_stress_test_rapid_scan_operations, "Test ServiceWifi - stress test: rapid scan operations", "[service][wifi][stress][scan]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_stress_test_continuous_state_transitions, "Test ServiceWifi - stress test: continuous state transitions", "[service][wifi][stress][state]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_stress_test_multiple_concurrent_operations, "Test ServiceWifi - stress test: multiple concurrent operations", "[service][wifi][stress][concurrent]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_stress_test_random_operations_serial, "Test ServiceWifi - stress test: random operations serial", "[service][wifi][stress][random][serial]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_stress_test_random_operations_parallel, "Test ServiceWifi - stress test: random operations parallel", "[service][wifi][stress][random][parallel]")
{
...
BROOKESIA_TEST_CASE(test_servicewifi_stress_test_long_running_operations, "Test ServiceWifi - stress test: long running operations", "[service][wifi][stress][long_running]")
{
