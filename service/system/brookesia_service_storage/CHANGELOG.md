...
- fix(filesystem): use lexical path normalization on ESP to avoid unsafe flash metadata access.
...
- break(repo): rename NVS service to Storage service and publish it under the system service category

### Enhancements:

- feat(storage): add ESP and PC build split for the renamed Storage service component
- feat(storage): keep namespace key-value APIs while integrating system storage workflows

### Documentation:

- docs(readme): refresh Storage README and test app documentation for the renamed component
